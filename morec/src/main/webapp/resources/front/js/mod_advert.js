					...
					d.prototype,
					{
						...
						general : function() {
							...
							for ( var f = 0; f < this.len; f++) {
								...
								k.push("<li><a" + ' href="' + h.link
										+ '" target="_blank" hotName="'
										+ (h.hotName || "") + '"><img title="'
										+ m + '" alt="' + m + '"  ksrc="' + h.src + '"></a></li>')
							}
							k.push("</ul>");
							if (g.showBar) {
								...
								for ( var f = 0; f < this.len; f++) {
									...
									k.push('<li class="item'+ f + '">'
													+ (g.barLink ? ("<a " + (g.openTarget !== "_self" ? (' target="' + g.openTarget + '"') : "")) + ' href="' + h.link + '">': "")
													+ (g.barNumber ? (f + 1) : h.text)
